...
def leadership(request):
	return render(request, 'sarznewapp/leadership.html')

def home(request):
	return render(request, 'sarznewapp/index.html')

def plan(request):
	return render(request, 'sarznewapp/plan.html')

def otherpayment(request):
    return render(request, 'sarznewapp/otherpayment.html')


def about(request):
	return render(request, 'sarznewapp/company.html')


def career(request):
	return render(request, 'sarznewapp/career.html')
...
def signin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				print(email)
				template= render_to_string('sarznewapp/loginAlert.html', {'name':username})
	...
	context={}
	return render(request, 'sarznewapp/login.html')


def privacy(request):
	return render(request, 'sarznewapp/privacy.html')
...
def main_view(request, *args, **kwargs):
	...
	return render(request, 'sarznewapp/main.html')
...
def signup(request):
	...
	if form.is_valid():
		if client_id is not None:
			...
			referral_template= render_to_string('sarznewapp/referalsignupmail.html', {'name':recommended_by_client_name, 'refereed':username})
		...
		template= render_to_string('sarznewapp/welcomeEmail.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
			'Welcome to universalassetventures',
			plain_message,
			settings.EMAIL_HOST_USER,
			[email],
		...
		second_template= render_to_string('sarznewapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
		second_plain_message= strip_tags(second_template)
		second_email_message= EmailMultiAlternatives(
			"Stay updated and discover more with universalassetventures!",
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['support@uaventures.cloud'])
		...
		return redirect('dashboard')
	context={'form':form}
	return render(request, 'sarznewapp/register.html', context)
...
def dashboard(request):
	...
	return render(request, 'sarznewapp/dashboardtwo.html', context)
...
def admindashboard(request):
	...
	return render(request, 'sarznewapp/adminpage.html', context)


def terms(request):
	return render(request, 'sarznewapp/terms.html')


def realestate(request):
	return render(request, 'sarznewapp/realestate.html')


def crypto(request):
	return render(request, 'sarznewapp/crypto.html')


def forex(request):
	return render(request, 'sarznewapp/forex.html')


def nft(request):
	return render(request, 'sarznewapp/nft.html')


def contact(request):
	return render(request, 'sarznewapp/contact.html')


def stocks(request):
	return render(request, 'sarznewapp/stocks.html')


def agriculture(request):
	return render(request, 'sarznewapp/agriculture.html')


def gold(request):
	return render(request, 'sarznewapp/gold.html')


def retirement(request):
	return render(request, 'sarznewapp/retirement.html')
...
def deposit(request):
	if request.method=='POST':
		...
		order_id= 'Universal Asset Ventures'
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
				...
				"success_url": "https://www.universalassetventures.org/dashboard",
				#our success url will direct us to the get_payment_status view for balance top ups
				"cancel_url": "https://www.universalassetventures.org/dashboard"
			})
			headers={'x-api-key':'RJ3H845-MV94GSW-PEWAM75-5TW2EH0', 'Content-Type': 'application/json'}
			...
			template= render_to_string('sarznewapp/pendingDepositEmail.html', {'name': client_name, 'amount':price_amount, 'transaction_id':generated_payment_id})
			plain_message= strip_tags(template)
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['support@uaventures.cloud'])
			except BadHeaderError:
				pass
			return redirect(generated_link)
	context={}
	return render(request, 'sarznewapp/deposittwo.html', context)

'''
@login_required(login_url='signin')
def investmentPlans(request):
	return render(request, 'sarznewapp/investmentplans.html')

@login_required(login_url='signin')
def retirementDeposit(request):
	form = RetirementDepositForm(request.POST or None)
	if form.is_valid():
	...
	context= {'form':form}
	return render(request, 'sarznewapp/retirementdepositForm.html', context)

@login_required(login_url='signin')
def realestateDeposit(request):
	form = RealestateDepositForm(request.POST or None)
	if form.is_valid():
		investment_info= form.cleaned_data.get('investment_plan')
		price_amount= form.cleaned_data.get('amount')
		pay_currency= form.cleaned_data.get('pay_currency')
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'sarznewapp/realestateDepositForm.html', context)
...
def cryptoDeposit(request):
	...
	if form.is_valid():
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'sarznewapp/cryptoDepositForm.html', context)

@login_required(login_url='signin')
def stocksDeposit(request):
	form = StocksDepositForm(request.POST or None)
	if form.is_valid():
		investment_info= form.cleaned_data.get('investment_plan')
		price_amount= form.cleaned_data.get('amount')
		pay_currency= form.cleaned_data.get('pay_currency')
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'sarznewapp/StocksDepositForm.html', context)
...
def portfolioDeposit(request):
	...
	if form.is_valid():
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'sarznewapp/PortfolioDepositForm.html', context)
...
def deposit(request):
	...
	order_id= 'universalassetventures'
	order_description= "This is a plan subscription"
	if price_amount and pay_currency:
		# Api's url link
		url= 'https://api.nowpayments.io/v1/invoice'
		...
		headers={'x-api-key':'8BY8Q7F-FZPMWBQ-H7F934H-KZTZ9FH', 'Content-Type': 'application/json'}
		response= requests.request('POST', url, headers=headers, data=payload)
		res= response.json()
		print(res)
		generated_link= res["invoice_url"]
...
def withdrawal(request):
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['support@uaventures.cloud'])
	...
	context={}
	return render(request, 'sarznewapp/withdrawal2.html', context)
...
def history(request):
	...
	return render(request, 'sarznewapp/history.html', context)
...
def pending_deposit(request):
	...
	return render(request, 'sarznewapp/pending_deposit.html', context)
...
def pending_withdrawal(request):
	...
	return render(request, 'sarznewapp/pending_withdrawal.html', context)
...
def pending_bonus(request):
	...
	return render(request, 'sarznewapp/pending_bonus.html', context)
...
def completed_transaction(request):
	...
	return render(request, 'sarznewapp/completed_transaction.html', context)
...
def myreferals(request):
    ...
    return render(request, 'sarznewapp/referralprofilestwo.html', context)
...
def confirm_withdrawal(request):
	...
	return render(request, 'sarznewapp/confirmwithdrawal.html', context)
...
def update_withdrawal(request, pk):
	...
	client_email= client.email_address
	client_withdrawal= client.withdrawal
	template= render_to_string('sarznewapp/withdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def decline_wihdrawal(request, pk):
	...
	template= render_to_string('sarznewapp/declineWithdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def confirm_deposit(request):
	...
	return render(request, 'sarznewapp/confirmdeposit.html', context)
...
def update_payment(request, pk):
	...
	template= render_to_string('sarznewapp/confirmDepositEmail.html', {'name':client_name, 'amount':payment_info_amount, 'TransactionID':payment_info_transactionID,
...
def account_settings(request):
	...
	context= {"form":form}
	return render(request, 'sarznewapp/account_settings.html', context)
...
def notifications(request):
	...
	return render(request, 'sarznewapp/notifications.html', context)
...
def transfer_funds(request):
		...
		if float(client_balance) > float(amount):
			try:
				...
				if destination_client_account_info.username != client_username:
					...
					template= render_to_string('sarznewapp/creditMail.html', {'name':destination_client_account_info.username,
					...
					emailmessage.attach_alternative(template, 'text/html')
					emailmessage.send()
					debit_template= render_to_string('sarznewapp/debitMail.html', {'name':client_username,
					...
					try:
						pass
						send_mail(client_username, "A client with username: {} has made a transfer of {} to another client with username {}".format(client_username, amount, destination_client_account_info.username),settings.EMAIL_HOST_USER, ['support@uaventures.cloud'])
					except BadHeaderError:
						pass
						return HttpResponse('Something went wrong, please try again later')
	...
	context= {}
	return render(request, 'sarznewapp/transferFunds.html', context)
...
def video_page(request):
	...
	return render(request, 'sarznewapp/videoPage.html', context)

@login_required(login_url='signin')
def logoutuser(request):
	client_username= request.user.username
	client_email= request.user.email
	template= render_to_string('sarznewapp/logoutMail.html', {'name':client_username})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
		'Logout Notification',
		plain_message,
...
def reinvest(request):
	...
	if request.method == 'POST':
		...
		if (float(client_bal) > float(amount)):
			...
			template= render_to_string('sarznewapp/reinvestmail.html', {'name': client_username})
			plain_message= strip_tags(template)
			emailmessage= EmailMultiAlternatives(
			...
			emailmessage.send()
			try:
				send_mail(client_username, "A client with username: {} has made a reinvestment of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['support@uaventures.cloud'])
			except BadHeaderError:
				pass
	...
	return render(request, 'sarznewapp/reinvest.html', context)
...
def loan(request):
	if request.method =='POST':
		...
		amount= request.POST.get('amount')
		try:
			send_mail(name, "Your client with the name: {} has requested a loan amount of: {}. Their email is {} and phone number is {}".format(name, amount,email, phone),email, ['support@uaventures.cloud'])
	...
	return render(request, 'sarznewapp/loan.html')
...
def portfolio(request):
	...
	return render(request, 'sarznewapp/portfolio.html', context)